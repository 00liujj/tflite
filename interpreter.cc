...
TfLiteStatus Interpreter::Invoke() {
  ...
  for (int execution_plan_index = 0;
    ...
    printf("node %d, opid %d, type %s, time %f\n",
           node_index,
           registration.builtin_code,
           registration.custom_name ? registration.custom_name : EnumNamesBuiltinOperator()[registration.builtin_code],
           1e3*float(stop_time-start_time)/CLOCKS_PER_SEC);
  }
